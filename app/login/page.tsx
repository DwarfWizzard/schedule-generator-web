...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetchClient<TokenPair>('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ username, password }),
      });

      const access = response.response?.access_token;
      const refresh = response.response?.refresh_token;
      ...
      setError(err.apiMessage || 'Ошибка авторизации');
    } finally {
      setLoading(false);
    }
  };
